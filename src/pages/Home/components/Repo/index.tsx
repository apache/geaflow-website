        ...
        <div className={styles.repoCardList}>
            {
                repos.map((item) => <div key={item.repo} className={styles.repoCard}>
                    ...
                    <div className={styles.repoCardInfo}>
                        ...
                        {item?.stargazers_count ? <div className={styles.herf} onClick={() => onLink(item.repo, 'stargazers')}><img src={'/img/collect.svg'} alt="" /> {item.stargazers_count}</div> : null}
                        {item?.forks_count ? <div className={styles.herf} onClick={() => onLink(item.repo, 'forks')}><img src={'/img/fork.svg'} alt="" /> {item.forks_count}</div> : null}
                    </div>
                </div>)
            }
        </div>
